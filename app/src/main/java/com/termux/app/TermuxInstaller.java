...
final class TermuxInstaller {
    ...
    private static void initFiles(final Activity activity) {
        AssetCopy copy = new AssetCopy(activity);
        try {
            File file = activity.getFilesDir();
            String homePath = new File(file, "home").getAbsolutePath();
            String binPath = new File(file, "usr/bin").getAbsolutePath();
            copy.copy("ubuntu-mantic-aarch64-pd-v4.8.0.tar.xz", homePath);
            copy.copy("wine.deb", homePath);
            copy.copy("exagear.zip", homePath);
            copy.copy("setup", binPath);
            copy.copy("setup-ubuntu", binPath);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
...
//                    File file = new File(TermuxService.FILES_PATH+"/bootstrap-aarch64.zip");
//                    final URL zipUrl = determineZipUrl();
//                    try (ZipInputStream zipInput = new ZipInputStream(zipUrl.openStream())) {
                    InputStream is = activity.getAssets().open("bootstrap-aarch64.zip");
                    try (ZipInputStream zipInput = new ZipInputStream(is)) {
                        ZipEntry zipEntry;
                    ...
                    Log.e(LOG_TAG, "Bootstrap error", e);
